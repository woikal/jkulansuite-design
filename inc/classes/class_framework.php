...
class framework
{
    ...
    var $facebook_meta_properties = "";     // meta tags for facebook
    ...
    function framework()
    {
        ...
        $this->facebook_meta_properties = '<meta property="og:image" content="' . $_SERVER['SERVER_NAME'] . '/ext_inc/logo/jku-lan.png" />';
    }
    ...
    /**
     * Set facebook meta properties
     *
     * @param string Newsbox title
     * @param string Newsbox description
     * @param string image path
     */
    function add_facebook_meta_props($title, $description, $image = '')
    {

        $default_image = $this->facebook_meta_properties;

        $server = $_SERVER['HTTPS'] ? 'https://' : 'http://';
        $server .= $_SERVER['SERVER_NAME'];
        $server .= ':' . $_SERVER['SERVER_PORT'];

        $this->facebook_meta_properties = '';
        $this->facebook_meta_properties .= '<meta property = "og:url" content="' . $server . '/index.php?' . $this->internal_url_query['query'] . '" />' . PHP_EOL;
        $this->facebook_meta_properties .= '<meta property = "og:title" content="' . $title . '" />' . PHP_EOL;
        $this->facebook_meta_properties .= '<meta property = "og:description" content="' . $description . '" />' . PHP_EOL;

        if ($image !== '' /*&& is_readable($server.$image)*/)
        {
            $img_info = getimagesize($image);
            $width    = 1200; // optimzed by 2017 values
            $height   = $img_info[1] * 1200 / $img_info[0]; // relative to width

            $this->facebook_meta_properties .= '<meta property = "og:image" content="' . $server . $image . '" />' . PHP_EOL;
            $this->facebook_meta_properties .= '<meta property = "og:image:width" content="' . $width . '" />' . PHP_EOL;
            $this->facebook_meta_properties .= '<meta property = "og:image:height" content="' . $height . '" />' . PHP_EOL;
        }
        else
        {
            $this->facebook_meta_properties .= $default_image;
        }
...
// Add special CSS and JS
        ...
        $smarty->assign('facebook_meta_properties', $this->facebook_meta_properties);
        ...
        if ($cfg['google_analytics_id'])
        {
            ...
            $EndJS .= '  var _gaq = _gaq || [];';
            $EndJS .= '  _gaq.push([\'_setAccount\', \'' . $cfg['google_analytics_id'] . '\']);';
            $EndJS .= '  _gaq.push([\'_trackPageview\']);';
            $EndJS .= '  (function() {';
            $EndJS .= '    var ga = document.createElement(\'script\');';
            $EndJS .= '    ga.type = \'text/javascript\'; ga.async = true;';
            $EndJS .= '    ga.src = (\'https:\' == document.location.protocol ? \'https://ssl\' : \'http://www\') + \'.google-analytics.com/ga.js\';';
            $EndJS .= '    var s = document.getElementsByTagName(\'script\')[0];';
            $EndJS .= '    s.parentNode.insertBefore(ga, s);';
            $EndJS .= '  })();';
            $EndJS .= '</script>';
        }
        ...
        switch ($this->modus)
        {
            ...
            default :
                ...
                if (!$_SESSION['lansuite']['fullscreen'] and !$this->modus == 'beamer')
                {
                    ...
                    if ($auth['type'] >= 2 and isset($debug))
                    { // and $cfg['sys_showdebug'] (no more, for option now in inc/base/config)
