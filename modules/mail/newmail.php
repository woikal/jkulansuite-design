...
function SendOnlineMail()
{
    ...
    if ($_POST['toUserID'] == -1) {
        ...
        if ($cfg['sys_internet']) {
            ...
            if ($row['lsmail_alert']) {
                $mail->create_inet_mail($row['username'], $row['email'], 'Benachrichtigung: Neue LAN-Mail', 'Du hast eine neue LAN-Mail erhalten. Diese Benachrichtigung kannst du im System unter "Meine Einstellungen" deaktivieren');
            }
        }
        return true;
