...
class boxes
{
    ...
    /**
     * Start a new menu built of menuitems in the navigation bar
     *
     * @param integer level of navigation (0=main, 1=sub, etc)
     * @param integer active / higlighted
     * @return void
     */
    function addMenuStart($level = 0, $newline = false)
    {
        $class = '';
        for ($l = 0; $l < $level; $l++) $class .= 'sub';
        $class .= 'menu';

        $this->box_rows .= ($newline ? "\n\t\t\t" : '') . '<ul class="' . $class . '">' . "\n";
    }

    /**
     * End the last menu built of menuitems in the navigation bar
     */
    function addMenuEnd($level = 0, $active = false)
    {
        $this->box_rows .= "</ul>\n";
    }

    /**
     * Add a menu item to the navigation bar
     *
     ...
     * @return void
     */
    function addMenuItem($caption, $link = '', $hint = '', $level = 0, $permissionlevel = 0, $active = false, $id = '', $hasSubItems = false)
    {
        global $func;
        ...
        if ($link != "") {
            if ($hint) $infobox = '<div class="infobox">' . $func->AllowHTML($hint) . '</div>';
            $tmp_link = '<a href="' . $func->AllowHTML($link) . '" class="' . $link_class . '">' . $caption . $infobox . '</a>';
        }
        if (strip_tags($caption) == $caption) $caption = wordwrap($caption, 18, "<br />\n", 1);
        if ($id) $id = ' id="' . $id . '"';
        $this->box_rows .= "\t\t<li" . $id . ' class="' . $class . '">' . $tmp_link;
    }
    ...
    function LinkItem($link, $caption, $class = "", $hint = '')
    {
        ...
        if ($link != "") {
            if ($hint) $infobox = '<div class="infobox">' . $func->AllowHTML($hint) . '</div>';
            $html = '<a href="' . $link . '" class="' . $class . '">' . $caption . $infobox . '</a>';
            return $html;
        } else return $caption;
    }
