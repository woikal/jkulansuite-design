...
// Fetch Boxes
$MenuActive = 0;
$BoxRes     = $db->qry("SELECT boxid, name, place, source, module, callback, login, internet 
                        FROM %prefix%boxes
                        WHERE active = 1
                          AND (internet = 0 OR internet = %int% + 1)
                          AND (login = 0 
                              OR (login = 1 AND %int% = 0) 
                              OR (login = 2 AND %int% = 1) 
                              OR (login > 2 AND login <= %int% + 1))
                        ORDER BY pos",
...
while ($BoxRow = $db->fetch_array($BoxRes))
    if (($BoxRow['module'] == '' or $func->isModActive($BoxRow['module'])) and ($BoxRow['callback'] == '' or call_user_func($BoxRow['callback'], '')))
    {
        ...
        if ($BoxRow['source'] == 'menu')
        {
            ...
            /* alternative navigation ::woikerl */
            $templ['index']['control']['navigation'] = $menu->getNavigation();
